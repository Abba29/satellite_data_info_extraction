...
"""
    ...
    (3) NDGI, Norm G, Norm NIR, Norm R
    (4) GDVI, NDSI, RI, IO, CI
"""
choice = 1
...
for date in dates:
    ...
    plt.tight_layout()
    plt.savefig("../" + date + "_planet_correlation_indices_" + str(choice) + ".png", dpi=300)  # save plot to .png
    plt.close()
    
if choice == 1 or choice == 2 or choice == 3 or choice == 4:
    # save the correlation matrices to .xlsx file [one matrix (i.e. one date) per sheet]
    with pd.ExcelWriter("../planet_correlation_indices_" + str(choice) + ".xlsx") as writer:
    ...
    with pd.ExcelWriter("../planet_correlation_indices_" + str(choice) + "_stats.xlsx") as writer:
