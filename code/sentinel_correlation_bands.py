...
for date in dates:
    ...
    plt.tight_layout()
    # plt.savefig("../" + date + "_sentinel_correlation_bands.png", dpi=300)  # save plot to .png
    plt.close()

# save the correlation matrices to .xlsx file [one matrix (i.e. one date) per sheet]
with pd.ExcelWriter("../sentinel_correlation_bands.xlsx") as writer:
...
with pd.ExcelWriter("../sentinel_correlation_bands_stats.xlsx") as writer:
