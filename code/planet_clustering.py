...
for i in range(0, len(planet_dfs)):
    temp_df = pd.DataFrame()
    for band in bands:
        temp_df.insert(len(temp_df.axes[1]), band, pd.Series(planet_dfs[i][band].mean()))
...
for i in range(0, len(planet_dfs)):
    temp_df = pd.DataFrame()
    for band in bands:
        diff = planet_dfs[i][band] - mean_dfs_repeated[i][band]
...
# with pd.ExcelWriter("../planet_temporal_mean_of_difference_values.xlsx") as writer:
