...
for date in dates:
    ...
    temp_planet_indices_ndarr = planet3D.sel(date=date)
    temp_planet_indices_df = pd.DataFrame(temp_planet_indices_ndarr.data, columns=bands)
    
    temp_planet_indices_ndarr2 = planet3D.sel(date=date)
    temp_planet_indices_df2 = pd.DataFrame(temp_planet_indices_ndarr2.data, columns=bands)
    
    if choice == 0:
        print("No indices calculus")    
    elif choice == 1:
        calc_ndvis(temp_planet_indices_df)
    elif choice == 2:
        calc_nvdi_corrections(temp_planet_indices_df)
    elif choice == 3:
        calc_other_vegetation_indices(temp_planet_indices_df)
    elif choice == 4:
        calc_soil_indices(temp_planet_indices_df)
    else:
    ...
    if choice2 == 0:
        ...
        calc_ndvis(temp_planet_indices_df2)
    elif choice2 == 2:
        calc_nvdi_corrections(temp_planet_indices_df2)
    elif choice2 == 3:
        calc_other_vegetation_indices(temp_planet_indices_df2)
    elif choice2 == 4:
        calc_soil_indices(temp_planet_indices_df2)
    else:
        print("Incorrect number. \n No index will be calculated. \n Calculating correlation with bands...")
    ...
    for band in bands:
        temp_planet_indices_df.drop(columns=band, inplace=True)
        temp_planet_indices_df2.drop(columns=band, inplace=True)
    ...
    # normalize/standardize planet data
    ...
    indices_col_names = temp_planet_indices_df.columns
    temp_planet_indices_df = scaler.fit_transform(temp_planet_indices_df)
    temp_planet_indices_df = pd.DataFrame(temp_planet_indices_df, columns=indices_col_names)
    
    # normalize/standardize indices data
    indices_col_names2 = temp_planet_indices_df2.columns
    temp_planet_indices_df2 = scaler.fit_transform(temp_planet_indices_df2)
    temp_planet_indices_df2 = pd.DataFrame(temp_planet_indices_df2, columns=indices_col_names2)
    
    soil_dfs.append(temp_soil_df)
    planet_dfs.append(temp_planet_df)
    indices_dfs.append(temp_planet_indices_df)
    indices_dfs2.append(temp_planet_indices_df2)

# convert each DataFrame into a Numpy array
soil_df_arrays = [df.to_numpy() for df in soil_dfs]
planet_df_arrays = [df.to_numpy() for df in planet_dfs]
...
planet_norm_matrix = np.array(planet_df_arrays)
...
planet_norm3D = xr.DataArray(planet_norm_matrix, coords=[dates, points, bands], dims=["date", "point", "band"])
...
# tmp2 = planet_norm3D.sel(point=pnt, band=bnd)
...
plt.tight_layout()
plt.savefig("../planet_latest.png", dpi=300)  # save plot to .png
plt.close()